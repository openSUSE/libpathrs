...
pub(crate) trait FdExt: AsFd {
    ...
    fn get_fdinfo_field_unchecked<T: FromStr>(
        &self,
...
mod tests {
    ...
    #[test]
    fn get_fdinfo_field_unchecked() -> Result<(), Error> {
        let file = File::open("/").context("open dummy file")?;

        assert_eq!(
            file.get_fdinfo_field_unchecked::<u64>(RawProcfsRoot::UnsafeGlobal, "pos")?,
            Some(0),
            "pos should be parsed and zero for new file"
        );

        assert_eq!(
            file.get_fdinfo_field_unchecked::<String>(RawProcfsRoot::UnsafeGlobal, "flags")?,
            Some("02100000".to_string()),
            "flags should be parsed for new file"
        );

        assert_ne!(
            file.get_fdinfo_field_unchecked::<u64>(RawProcfsRoot::UnsafeGlobal, "mnt_id")?
        ...
        assert_eq!(
            file.get_fdinfo_field_unchecked::<u64>(RawProcfsRoot::UnsafeGlobal, "non_exist")?,
            None,
    ...
    #[test]
    fn get_fdinfo_field_unchecked_proc_rootfd() -> Result<(), Error> {
        let procfs = ProcfsHandle::new().context("open procfs handle")?;
        let file = File::open("/").context("open dummy file")?;

        assert_eq!(
            file.get_fdinfo_field_unchecked::<u64>(procfs.as_raw_procfs(), "pos")?,
            Some(0),
            "pos should be parsed and zero for new file"
        );

        assert_eq!(
            file.get_fdinfo_field_unchecked::<String>(procfs.as_raw_procfs(), "flags")?,
            Some("02100000".to_string()),
            "flags should be parsed for new file"
        );

        assert_ne!(
            file.get_fdinfo_field_unchecked::<u64>(procfs.as_raw_procfs(), "mnt_id")?
                .expect("should find mnt_id in fdinfo"),
            0,
            "mnt_id should be parsed and non-nil for any real file"
        );
        ...
        assert_eq!(
            file.get_fdinfo_field_unchecked::<u64>(procfs.as_raw_procfs(), "non_exist")?,
            None,
            "non_exist should not be present in fdinfo"
        );
